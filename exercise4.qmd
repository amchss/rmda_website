...
- Filter the dataset to find only the records of mothers who smoke (`smoke` variable).
  ...
  - *Hint:* Use `filter()` to select rows where `smoke == 1` (assuming `1` represents smokers).
...
### **Question 9:** Create a New Variable for BMI Category

- Create a new variable `bmi_category` based on the mother's BMI (`bmi`). You can categorize `bmi` into three categories: "Underweight" (BMI < 18.5), "Normal weight" (18.5 ≤ BMI < 24.9), and "Overweight" (BMI ≥ 25).
...
### **Question 10:** Count the Number of Babies by BMI Category and Smoking Status

- Group the dataset by both `bmi_category` and `smoke` (whether the mother smokes or not), and count the number of babies in each group.
...
### **Question 11:** Filter Babies Born with Low Birth Weight and Mother's BMI Over 30

- Filter the dataset to find babies who were born with a low birth weight (`bwt` < 2500g) and whose mother has a BMI greater than 30.
